...
from items import WeiboItem_v1, UserItem_v1
...
class MongodbPipeline(object):
    ...
    def __init__(self, host=MONGOD_HOST, port=MONGOD_PORT):
        self.db = _default_mongo(host, port, usedb='master_timeline_v1')
        log.msg('Mongod connect to {host}:{port}'.format(host=host, port=port), level=log.INFO)

    @classmethod
    def from_settings(cls, settings):
        host = settings.get('MASTER_TIMELINE_V1_MONGOD_HOST', MONGOD_HOST)
        port = settings.get('MASTER_TIMELINE_V1_MONGOD_PORT', MONGOD_PORT)
    ...
    def process_item(self, item, spider):
        if isinstance(item, WeiboItem_v1):
            return deferToThread(self.process_weibo, item, spider)
        elif isinstance(item, UserItem_v1):
    ...
    def process_weibo(self, item, spider):
        ...
        if self.db.master_timeline_weibo.find({'_id': weibo['_id']}).count():
            ...
            for key in WeiboItem_v1.PIPED_UPDATE_KEYS:
    ...
    def process_user(self, item, spider):
        ...
        if self.db.master_timeline_user.find({'_id': user['_id']}).count():
            updates = {}
            updates['last_modify'] = time.time()
            for key in UserItem_v1.PIPED_UPDATE_KEYS:
                # 如followers这一项，初始化为[]
