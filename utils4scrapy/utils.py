...
import socket
from items import WeiboItem, UserItem, WeiboItem_v1, UserItem_v1


def resp2item_v1(resp, base_weibo=None, base_user=None):
    items = []
    ...
    if 'mid' in resp:
        weibo = WeiboItem_v1()
        for key in WeiboItem_v1.RESP_ITER_KEYS:
            if key in resp:
                weibo[key] = resp[key]
        ...
        items.append(weibo)
        items.extend(resp2item_v1(resp.get('user'), base_weibo=weibo))
        items.extend(resp2item_v1(resp.get('retweeted_status'), base_weibo=weibo))
    else:
        user = UserItem_v1()
        for key in UserItem_v1.RESP_ITER_KEYS:
            user[key] = resp[key]
        user['timestamp'] = local2unix(user['created_at'])
        
        if base_weibo:
            base_weibo['user'] = user

        items.append(user)
        items.extend(resp2item_v1(resp.get('status'), base_user=user))
